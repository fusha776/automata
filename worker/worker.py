...
class Worker():
    ...
    def check_action_blocked(self):
        ...
        if el:
            # body = self.wf.pixel.driver.find_elements_by_id('com.instagram.android:id/dialog_body')
    ...
    def run(self):
        ...
        try:
            ...
            # いつものエラーを出力
            # t, v, tb = sys.exc_info()
            # self.facade.abilities.logger.debug(traceback.format_exception(t, v, tb))
            ...
            self.facade.abilities.logger.debug(f'automata を終了します. worker_id: {worker_id}, login id: {login_id}')
            sleep(120)
